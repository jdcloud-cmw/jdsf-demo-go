...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	ext.HTTPMethod.Set(tracer.CurrentSpan, req.Method)
	ext.HTTPUrl.Set(tracer.CurrentSpan, req.URL.String())

	carrier := opentracing.HTTPHeadersCarrier(req.Header)
	tracer.CurrentSpan.Tracer().Inject(tracer.CurrentSpan.Context(), opentracing.HTTPHeaders, carrier)
	...
	ext.HTTPStatusCode.Set(tracer.CurrentSpan, uint16(resp.StatusCode))
	if req.Method == "HEAD" {
		tracer.CurrentSpan.Finish()
	} else {
		resp.Body = closeTracker{resp.Body, tracer.CurrentSpan}
	}
	return resp, nil
}
...
func (h *Tracer) Start(req *http.Request) opentracing.Span {
	if h.RootSpan == nil {
		...
		root := h.Tracer.StartSpan(operationName, opentracing.ChildOf(spanctx))
		h.RootSpan = root
	}

	ctx := h.RootSpan.Context()
	h.CurrentSpan = h.Tracer.StartSpan("HTTP "+req.Method + " "+ req.URL.Path, opentracing.ChildOf(ctx))
	ext.SpanKindRPCClient.Set(h.CurrentSpan)

	componentName := h.Opts.componentName
	if componentName == "" {
		componentName = defaultComponentName
	}
	ext.Component.Set(h.CurrentSpan, componentName)

	return h.CurrentSpan
}
...
func (h *Tracer) getConn(hostPort string) {
	ext.HTTPUrl.Set(h.CurrentSpan, hostPort)
	h.CurrentSpan.LogFields(log.String("event", "GetConn"))
}

func (h *Tracer) gotConn(info httptrace.GotConnInfo) {
	h.CurrentSpan.SetTag("net/http.reused", info.Reused)
	h.CurrentSpan.SetTag("net/http.was_idle", info.WasIdle)
	h.CurrentSpan.LogFields(log.String("event", "GotConn"))
}

func (h *Tracer) putIdleConn(error) {
	h.CurrentSpan.LogFields(log.String("event", "PutIdleConn"))
}

func (h *Tracer) gotFirstResponseByte() {
	h.CurrentSpan.LogFields(log.String("event", "GotFirstResponseByte"))
}

func (h *Tracer) got100Continue() {
	h.CurrentSpan.LogFields(log.String("event", "Got100Continue"))
}

func (h *Tracer) dnsStart(info httptrace.DNSStartInfo) {
	h.CurrentSpan.LogFields(
...
func (h *Tracer) dnsDone(info httptrace.DNSDoneInfo) {
	...
	h.CurrentSpan.LogFields(fields...)
}
...
func (h *Tracer) wroteHeaders() {
	h.CurrentSpan.LogFields(log.String("event", "WroteHeaders"))
}

func (h *Tracer) wait100Continue() {
	h.CurrentSpan.LogFields(log.String("event", "Wait100Continue"))
}
...
func (h *Tracer) wroteRequest(info httptrace.WroteRequestInfo) {
	if info.Err != nil {
		...
		ext.Error.Set(h.CurrentSpan, true)
	} else {
		h.CurrentSpan.LogFields(log.String("event", "WroteRequest"))
	}
}
